...
xyz ak8975c_read_xyz(void)
{
  ...
  while (DTRDY == 0)
  {
    twi_read_bytes(AK8975C_ADDRESS, &reg, 1, &DTRDY);
  }
...
xyz ak8975c_fuze_mode_read(void)
{
  ...
  _delay_ms(1);
  reg = 0x10;
  twi_read_bytes(AK8975C_ADDRESS, &reg, 3, buffer);

  ASA.x = (double)buffer[0];
  ASA.y = (double)buffer[1];
  //	ASA.z=(double) buffer[2];
